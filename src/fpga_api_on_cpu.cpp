...
void FPGA::largeMM(const float* weight_mat, const float* input_mat, float* output, int num_input, int num_output, int num_matrix2)
{
  ...
  for(int i = 0; i < num_output; i += v_size_)
  {
	...
	// Set vmat to zero at once
	memset(m1, 0, data_size_M * sizeof(float));

	// 1) Assign a m1
        for(int row = 0; row < block_row; ++row)
        {
		for(int col_1 = 0; col_1 < block_col_1; ++col_1)
		{
			m1[row*v_size_ + col_1] = weight_mat[(i+row)*num_input + j + col_1];
		}
	}
	...
	// 2) Assign a m2
        for(int col_1 = 0; col_1 < block_col_1; ++col_1)
        {
		for(int col_2 = 0; col_2 < block_col_2; ++col_2)
		{
			m2[col_1*v_size_ + col_2] = input_mat[(j+col_1)*num_matrix2 + k + col_2];
		}
	}
...
void FPGA::largeMV(const float* large_mat, const float* input, float* output, int num_input, int num_output)
{
  ...
  for(int i = 0; i < num_output; i += m_size_)
  {
    ...
    {			
      ...
      // Set vec and mat to zero at once
      memset(vec, 0, data_size_ * sizeof(float));
...
void FPGA::convLowering(const std::vector<std::vector<std::vector<std::vector<float>>>>& cnn_weights,
  ...
  for(int n = 0; n < conv_channel; ++n)
  {
    for(int c = 0; c < input_channel; ++c)
    {
      for(int h = 0; h < conv_height; ++h)
      {
        ...
        {
          new_weights[n][c*(conv_height*conv_width) + (h*conv_width + w)] = cnn_weights[n][c][h][w];
        }
      }
    }
  }
  for(int c = 0; c < input_channel; ++c)
  {
    for(int h = 0; h < conv_height; ++h)
    {
      for(int w = 0; w < conv_width; ++w)
      {
        for(int oh = 0; oh < (input_height - conv_height + 1); ++oh)
        {
          for(int ow = 0; ow < (input_width - conv_width + 1); ++ow)
	  {
		new_inputs[c*(conv_height*conv_width) + h*conv_width + w][oh*(input_width-conv_width+1) + ow] = inputs[c][oh+h][ow+w];
	  }
        }
      }
    }
